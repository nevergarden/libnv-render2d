...
void nv_transform2d(nv_mesh2d_t * meshes, size_t mesh_count, GLuint * vbo) {
  ...
  for (int p = 0; p < mesh_count; p++) {
    ...
    if(mesh.rotation == 0) {
      ...
      float vec[16] = {
    -mesh.scale.x + mesh.position.x, -mesh.scale.y + mesh.position.y, 0.0f, 0.0f,
     mesh.scale.x + mesh.position.x, -mesh.scale.y + mesh.position.y, 1.0f, 0.0f,
     mesh.scale.x + mesh.position.x,  mesh.scale.y + mesh.position.y, 1.0f, 1.0f,
    -mesh.scale.x + mesh.position.x,  mesh.scale.y + mesh.position.y, 0.0f, 1.0f
  };
